...
//       // subjectsTaught: lecturerSubject,
//       status: lecturerStatus,
//       // available: lecturerStatus,
//     };
...
function LecturerManagement() {
  ...
  const fetchLecturers = async () => {
    try {
      const response = await api.get("/lecturer/all", {
        headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` },
      });
  ...
  const handleSave = async () => {
    ...
    console.log("Payload being sent:", JSON.stringify(newLecturer));

    try {
      await api.post("/lecturer/add", newLecturer, {
        headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` },
      });
      ...
      console.error("Error saving lecturer:", error.response?.data || error.message);
      toast.error(`Failed to save lecturer! ${error.response?.data?.message || "Check input fields."}`);
    }
  };

  const handleDelete = async (id) => {
    try {
      await api.delete(`/lecturer/delete/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` },
      });
  ...
  const toggleStatus = async (id, status) => {
    try {
      await api.put(`/lecturers/update-status/${id}`, { status: !status }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("jwt")}` },
      });
      toast.success("Lecturer status updated!");
      fetchLecturers();
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-blue-50 p-10">
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-4 bg-blue-100 p-8 rounded-md shadow-lg">
          <h2 className="text-2xl font-semibold text-gray-700 mb-6">Enter Lecturer Details</h2>
          <input type="text" value={lecturerName} onChange={(e) => setLecturerName(e.target.value)} placeholder="Enter lecturer name" className="w-full p-3 mb-3 border rounded-md" />
          <input type="email" value={lecturerEmail} onChange={(e) => setLecturerEmail(e.target.value)} placeholder="Enter email" className="w-full p-3 mb-3 border rounded-md" />
          <input type="text" value={lecturerDepartment} onChange={(e) => setLecturerDepartment(e.target.value)} placeholder="Enter department" className="w-full p-3 mb-3 border rounded-md" />
          <select value={lecturerSubject} onChange={(e) => setLecturerSubject(e.target.value)} className="w-full p-3 mb-3 border rounded-md">
            <option value="">Select a subject</option>
            {subjects.map((subject) => (
              <option key={subject.id} value={subject.subjectName}>{subject.subjectName} ({subject.code})</option>
            ))}
          </select>
          <label className="flex items-center mb-3">
            <input type="checkbox" checked={lecturerStatus} onChange={() => setLecturerStatus(!lecturerStatus)} className="mr-2" /> Active
          </label>
          <button onClick={handleSave} className="bg-blue-500 text-white px-4 py-2 rounded-md">Save</button>
        </div>
        <div className="col-span-8 bg-white p-8 rounded-md shadow-md">
          <h3 className="text-2xl font-semibold mb-4">All Lecturers</h3>
          <input type="text" placeholder="Search by name or email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-3 mb-4 border rounded-md" />
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-200">
            ...
            <tbody>
              {lecturers.map((lecturer) => (
                <tr key={lecturer.id}>
                  ...
                  <td className="p-4">{lecturer.email}</td>
                  <td className="p-4">{lecturer.department}</td>
                  <td className="p-4 ">{lecturer.subjectsTaught}</td>
                  <td className="p-4">
                    <input type="checkbox" checked={lecturer.status} onChange={() => toggleStatus(lecturer.id, lecturer.status)} />
                  </td>
                  <td className="p-4">
                    <button onClick={() => handleDelete(lecturer.id)} className="text-red-500">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
